...
window.viewStudentDetails = async function (id) {
    ...
    try {
        ...
        let backlogHtml = backlogs.length === 0
            ? '<p class="text-sm text-green-600 font-bold bg-green-50 p-3 rounded-lg"><i class="fas fa-check-circle mr-1"></i> No backlogs found.</p>'
            : '<ul class="space-y-2">' + backlogs.map(b => `<li class="text-sm bg-red-50 text-red-700 p-2 rounded-lg border border-red-100"><i class="fas fa-exclamation-circle mr-2"></i><strong>${b.subjectCode}</strong>: ${b.subjectName} (Sem ${b.semester})</li>`).join('') + '</ul>';
        ...
        contentDiv.innerHTML = `
            ...
            <div class="mb-2"><h4 class="font-bold text-gray-700 dark:text-gray-300 text-sm mb-2 uppercase tracking-widest border-b pb-1 dark:border-gray-700">Current Backlogs</h4>${backlogHtml}</div>
        `;
    } catch (e) {
