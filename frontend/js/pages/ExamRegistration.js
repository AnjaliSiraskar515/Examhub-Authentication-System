export default function ExamRegistration() {
    ...
    const loadExams = async () => {
        try {
            ...
            try {
                const profileRes = await fetch('http://localhost:8081/api/profile/info', {
            ...
            // ── 2. Fetch open exams
            const response = await fetch('http://localhost:8081/api/university/exams');
            ...
            try {
                const regResponse = await fetch(`http://localhost:8081/api/student/registrations`, {
                    headers: { 'Authorization': token ? `Bearer ${token}` : '' }
                });
                if (regResponse.ok) {
    ...
    const setupRegistrationLogic = (lateFeeActive) => {
        ...
        submitBtn.addEventListener('click', async () => {
            ...
            try {
                ...
                const response = await fetch('http://localhost:8081/api/student/exams/register', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'Authorization': token ? `Bearer ${token}` : ''
                ...
                try {
                    ...
                    const regRes = await fetch('http://localhost:8081/api/student/registrations', {
